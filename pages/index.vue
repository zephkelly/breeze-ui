<template>
    <div>
        <h1>testing</h1>

        <BreezeButton :variant="outline">
            <template #icon-right>
                <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#e8eaed"><path d="M172.31-180Q142-180 121-201q-21-21-21-51.31v-455.38Q100-738 121-759q21-21 51.31-21h219.61l80 80h315.77Q818-700 839-679q21 21 21 51.31v375.38Q860-222 839-201q-21 21-51.31 21H172.31Zm0-60h615.38q5.39 0 8.85-3.46t3.46-8.85v-375.38q0-5.39-3.46-8.85t-8.85-3.46H447.38l-80-80H172.31q-5.39 0-8.85 3.46t-3.46 8.85v455.38q0 5.39 3.46 8.85t8.85 3.46ZM160-240v-480 480Z"/></svg>
            </template>
            Hey
            <template #icon-left>
                <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#e8eaed"><path d="M172.31-180Q142-180 121-201q-21-21-21-51.31v-455.38Q100-738 121-759q21-21 51.31-21h219.61l80 80h315.77Q818-700 839-679q21 21 21 51.31v375.38Q860-222 839-201q-21 21-51.31 21H172.31Zm0-60h615.38q5.39 0 8.85-3.46t3.46-8.85v-375.38q0-5.39-3.46-8.85t-8.85-3.46H447.38l-80-80H172.31q-5.39 0-8.85 3.46t-3.46 8.85v455.38q0 5.39 3.46 8.85t8.85 3.46ZM160-240v-480 480Z"/></svg>
            </template>
        </BreezeButton>
        <ClientOnly>
            <div>
                <p>Current Theme: {{ $currentTheme }}</p>
                <p>Using System Preference: {{ $isSystemColorScheme ? 'Yes' : 'No' }}</p>
                <p>Explicit Setting: {{ $colorScheme || 'None (using system)' }}</p>
                <button @click="$toggleColorScheme">
                Toggle Theme
                </button>
                <button @click="$resetToSystem">Reset to System</button>
            </div>
        </ClientOnly>
    </div>
</template>

<script setup lang="ts">
const outline: BreezeButtonVariant = 'outline';
</script>

<style scoped>
h1 {
    font-size: 2rem;
    margin-bottom: 2rem;
}
</style>